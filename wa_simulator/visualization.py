...
class WAMatplotlibVisualization:
    ...
    class VehiclePlotter:
        ...
        def Initialize(self, q, input_q):
            ...
            out = self.q.get()
            if not isinstance(out, SimState):
...
class WAMatplotlibJupyterVisualization:
    """Matplotlib visualizer of the simulator world and the vehicle
    ...
    def __init__(self, vehicle, system):
        ...
        self.plotter = self.VehiclePlotter(SimState(
            self.outline,
            self.rr_wheel,
            self.rl_wheel,
            self.fr_wheel,
    ...
    def KeyPress(self, event):
        ...
        Args:
            event (plt.KeyEvent): the key event
        """
