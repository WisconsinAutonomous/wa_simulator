...
def _get_files(file_):
	return [os.path.splitext(f.name)[0] for f in os.scandir(os.path.dirname(file_)) if f.is_file() and _not_hidden(f.name)]

def _get_dirs(file_):
	return [os.path.splitext(f.name)[0] for f in os.scandir(os.path.dirname(file_)) if f.is_dir() and _not_hidden(f.name)]
